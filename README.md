...
- **데스크톱 상주 앱** — 윈도우 트레이(`tray/`, Electron)와 macOS 메뉴바(`macos/`, WKWebView)에 상주해 창을 닫아도 재생 유지 (아래 참고)
...
macos/                 macOS 메뉴바 앱 (Swift · WKWebView)
```
...
계속된다. 메뉴바에는 재생/정지·채널(곡)명 스트립이 상주한다. 웹뷰가 WebKit이라 라디오는
네이티브 HLS로 재생되며, EQ·앰프 음색은 iOS와 같은 제약으로 계기 표시만 동작한다
(턴테이블·테이프 음원에는 적용). 자세한 조작과 빌드는 [`macos/README.md`](macos/README.md) 참고.
