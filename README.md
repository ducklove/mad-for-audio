...
[2026-09-11 전체 기능·성능·UX 점검](docs/PROJECT_REVIEW_2026-09-11.md)

## 기능

- **첫 재생 안내** — 랙 상단의 **▶ 듣기**를 누르면 기본 KBS 1FM 또는 마지막 채널이 재생된다. 연결 중에는 같은 버튼으로 취소하고, 실패하면 다시 연결한다. 컴포넌트 전원을 껐다 켤 필요가 없다. 브라우저의 오디오 제한은 클릭 안에서 해제하며, 사용자가 끈 앰프는 별도의 **앰프 켜기** 동작으로 복귀한다.
...
- **86장·494곡 음반 카탈로그** — 클래식 72장·398곡, 재즈 13장·93곡, 기타 1장·3곡. 가요 분류는 선택할 수 있으며 현재 수록곡은 없다. 성악은 클래식에 포함하고, 블루스·록·팝·일렉트로니카·레게·월드 등의 세부 장르는 기타에 모으되 검색용 태그로 남긴다
...
- **단독 기기** — 랙에 물리지 않고 혼자 소리를 내는 세 대: **Victor V 축음기**(1907, 어쿠스틱 — 태엽·강철 바늘·놋쇠 나팔관, **음반 전용**), **금성 A-501 라디오**(1959, 국산 1호 5구 진공관, **방송 전용**), **Lasonic TRC-931 붐박스**(1985, 라우드니스 커브의 풍부한 저음 — FUNCTION 스위치로 **방송·음반 겸용**, 전면 5밴드 EQ가 실제 필터로 동작). 고르면 랙 여섯 유닛이 화면에서 내려가고 오디오도 EQ·앰프를 건너뛰어 기기 고유의 음향 경로로만 흐른다
...
listening-controls.js  첫 재생·연결 취소·재시도 안내의 DOM과 이벤트 수명주기
deck.js                카세트 데크·테이프 전송 로직
skins.js               튜너·앰프 SVG 스킨
component-skins.js     실물 레퍼런스 기반 추가 튜너·앰프·데크·턴테이블 SVG 카탈로그
solo-skins.js          단독 기기 SVG — Victor V 축음기, 금성 A-501 라디오, Lasonic TRC-931 붐박스
...
운영 주소는 `https://ducklove.duckdns.org:3689`다. CBS·EBS·YTN 편성표도 이 프록시를 쓴다.
인증서 기본 경로는 `/etc/letsencrypt/live/ducklove.duckdns.org/`이며 `TLS_CERT_NAME`으로
도메인을 바꿀 수 있다. 프로세스는 매시간 인증서 변경을 확인해 기존 연결을 유지하면서
반영한다. 인증서 발급·갱신은 서버의 기존 갱신 작업에서 수행한다.
...
- **트레이 우클릭** — 재생/일시정지, 보기 전환(튜너형·턴테이블형·오디오 시스템), 채널 선택, 볼륨, 로그인 시 자동 시작, 시작할 때 바로 재생
...
`chromium-autoplay` 프로젝트는 자동재생 제한을 켠 상태에서 첫 클릭·재시작·연결 취소를
검증한다. WebKit도 첫 재생과 취소 흐름을 검증한다. 정적 서버를 8123 포트에서 실행한 뒤
`cd tests`에서 `node audit-runtime.js current`를 실행하면 4배 CPU 감속 조건의
데스크톱·390px 화면 준비 시간, 레이아웃 이동, 유휴 작업량, axe 접근성 결과와 화면을
`tests/artifacts/runtime-audit/`에 저장한다. 외부 방송 실측과 구분되는 로컬 모의 측정이다.
