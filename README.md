...
- **곡별 자동 분리** — 예약마다 PC의 Qwen3-ASR·MOSS-Audio로 곡을 분리하고 불확실한 구간만 Gemini 3.8 Flash로 보완. 원본 보존, 곡명·작곡가·연주자 태그, 경계·정보 수정, 곡별 FLAC·ZIP 저장 ([설치·사용 안내](docs/RADIO_TRACK_ANALYSIS.md))
- **PC 서버 상시 녹음** — 기본 KBS 1FM, 여러 채널·요일·시간·프로그램 선택. 앱을 닫아도 PC가 녹음하고, KBS 재방송 제외와 원본/곡별 저장을 지원한다. 채널별 녹음은 동시에, GPU 분석은 순서대로 진행한다.
