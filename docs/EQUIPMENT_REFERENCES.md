...
| Lasonic TRC-931 (1980년대 중반) | [Lasonic TRC-931 실물 사진 — 사용자 제공] | 검은 사출 캐비닛 전폭을 가로지르는 3밴드(FM·AM·SW) 다이얼 스케일과 우상단 대형 크롬 TUNING 노브, 좌상단 노랑·청록 인디케이터 사각, 붉은 머리띠의 5-BAND GRAPHIC EQUALIZER, Auto Reverse 필기체와 ON/OFF 붉은 프레임·파란 BAND SELECTOR, 중앙 세로의 데크 A·COUNTER·LASONIC 로고·TRC-931 데크 B·피아노 키 6개, 크롬 3스포크(가로+X)가 가로지르는 대형 우퍼 두 발과 모서리 콘 트위터, 크롬 파이프 손잡이(세로 스트럿 + 엘보 페룰 + 중앙 골지 고무 그립 + 힌지 너클 브래킷) |
...
두 참고 사진은 모두 **오래 쓴 실물**이다 — Victor V는 산화해 검붉게 내려앉은 셸락 니스와
탁해진 도금을, A-501은 심하게 부식된 크롬 몰딩·황변한 다이얼 유리·거의 지워진 GoldStar
금박을 보여 준다. SVG도 그 상태를 기준으로 삼았다. 노후 표현의 물리적 근거와 구현
기법은 docs/MODEL-IDENTITY.md의 '세월의 표현'에 정리했다.
...
SC·PU·SW 각인도 실물 그대로이지만, 이 라디오는 방송만 받으므로 셀렉터는 SC와 SW만 오가고
PU(픽업 입력)는 연결된 기기가 없어 건너뛴다.
색상은 출시 5색 중 사진으로 확인되는 차콜과 민트 두 가지만 제공한다.

## 구현 원칙

- 공개 모델 표에는 현재 설정 화면에서 선택 가능한 20종만 싣는다. DT-540은 단일 고정 유닛이고,
  단독 기기 3종은 랙 컴포넌트가 아니므로 별도 표로 둔다.
